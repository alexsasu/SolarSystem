...
public class ZoomTarget : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (target_tag == 6)
        {
            ...
            Vector3 targetPosition = target.TransformPoint(new Vector3(0f, 0.8f, -2f));
            this.transform.position = Vector3.SmoothDamp(transform.position, targetPosition, ref velocity, smoothTime);
        }
        else if (target_tag == 7)
        {
            target = GameObject.Find("Saturn").transform;
            Vector3 targetPosition = target.TransformPoint(new Vector3(2f, 2.3f, -1f));
            this.transform.position = Vector3.SmoothDamp(transform.position, targetPosition, ref velocity, smoothTime);
        }
        else if (target_tag == 8)
        {
            target = GameObject.Find("Uranus").transform;
            Vector3 targetPosition = target.TransformPoint(new Vector3(0f, 0.8f, -3f));
            this.transform.position = Vector3.SmoothDamp(transform.position, targetPosition, ref velocity, smoothTime);
        }
